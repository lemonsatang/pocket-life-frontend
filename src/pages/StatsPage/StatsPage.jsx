      ...
      {labelPositions.map((pos, index) => (
        <div
          ...
          style={{
            ...
            fontWeight: 'bold',
            fontSize: '0.85rem',
            textShadow: '0 2px 4px rgba(0, 0, 0, 0.5), 0 0 2px rgba(0, 0, 0, 0.8)', // [추가 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 라벨 가독성을 위한 그림자 강화, 어디서: StatsPage.jsx 101번째 줄, 어떻게: textShadow 추가, 왜: 흰색 텍스트가 차트 위에서 잘 보이도록 하기 위해
        ...
        >
          <span style={{ display: 'block' }}>{pos.label}</span> {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 라벨 텍스트를 블록 요소로 변경하여 줄바꿈 가능하게 함, 어디서: StatsPage.jsx 123번째 줄, 어떻게: span에 display: block 스타일 추가, 왜: 라벨과 퍼센트를 줄바꿈하여 표시하기 위해 */}
          <span style={{ display: 'block' }}>{pos.percentage}%</span> {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 퍼센트를 별도 블록 요소로 분리하여 줄바꿈 표시, 어디서: StatsPage.jsx 124번째 줄, 어떻게: span에 display: block 스타일 추가하고 별도 요소로 분리, 왜: 라벨 아래에 퍼센트를 표시하기 위해 */}
        </div>
      ))}
    </div>
  );
...
const StatsPage = () => {
  ...
  const [periodType, setPeriodType] = useState('month'); // 'week' 또는 'month'
  ...
  const [currentDietDetail, setCurrentDietDetail] = useState({ normalDays: 0, overDays: 0, underDays: 0, totalPeriodDays: 0, dateStatusMap: { normal: [], over: [], under: [], noRecord: [] } }); // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: dateStatusMap에 noRecord 추가, 어디서: StatsPage.jsx 154번째 줄, 어떻게: dateStatusMap 초기값에 noRecord 배열 추가, 왜: 기록없음 날짜를 표시하기 위해
  ...
  const [prevDietDetail, setPrevDietDetail] = useState({ normalDays: 0, overDays: 0, underDays: 0, totalPeriodDays: 0, dateStatusMap: { normal: [], over: [], under: [], noRecord: [] } }); // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: dateStatusMap에 noRecord 추가, 어디서: StatsPage.jsx 162번째 줄, 어떻게: dateStatusMap 초기값에 noRecord 배열 추가, 왜: 기록없음 날짜를 표시하기 위해
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          // 신규 API 시도
          [resMealCurrent, resCartCurrent, resTodoCurrent, resCategoryCurrent] = await Promise.all([
            dataApi.get(`/api/stats/meal/range`, { params: { startDate: currentStart, endDate: currentEnd } }).catch((e) => {
              console.error("식단 통계 API 실패:", e);
              return { data: null };
            }),
            dataApi.get(`/api/stats/cart/range`, { params: { startDate: currentStart, endDate: currentEnd } }).catch((e) => {
              console.error("장바구니 통계 API 실패:", e);
              return { data: null };
            }),
            dataApi.get(`/api/todo/stats`, { params: { startDate: currentStart, endDate: currentEnd } }).catch((e) => {
              console.error("일정 통계 API 실패:", e);
              return { data: null };
            }),
            dataApi.get(`/api/tx/category-stats`, { params: { startDate: currentStart, endDate: currentEnd } }).catch((e) => {
        ...
        if (!resMealCurrent?.data || !resCartCurrent?.data || !resTodoCurrent?.data) {
          ...
          for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
            ...
            dataApi.get(`/api/todo/getList`, { params: { date: dateStr } }).catch(() => ({ data: [] })),
                dataApi.get(`/api/tx`, { params: { year: d.getFullYear(), month: d.getMonth() + 1 } }).catch(() => ({ data: [] }))
              ]);
              ...
              if (meals.data && meals.data.totalCalories !== undefined) {
                if (!resMealCurrent || !resMealCurrent.data) {
                  resMealCurrent = { data: { totalCalories: 0, targetCalories: 0 } };
                }
                resMealCurrent.data.totalCalories += (Number(meals.data.totalCalories) || 0);
                resMealCurrent.data.targetCalories += (Number(meals.data.targetCalories) || 2000);
                console.log(`DEBUG: 날짜 ${dateStr} 식단 통계`, meals.data, "누적:", resMealCurrent.data);
              }
              ...
              if (cart.data && cart.data.totalQuantity !== undefined) {
                if (!resCartCurrent?.data) {
                  resCartCurrent = { data: { purchaseRate: 0, totalQuantity: 0, purchasedQuantity: 0 } };
                }
                resCartCurrent.data.totalQuantity += (Number(cart.data.totalQuantity) || 0);
                resCartCurrent.data.purchasedQuantity += (Number(cart.data.purchasedQuantity) || 0);
              }
              
              if (todos.data) todoList.push(...(Array.isArray(todos.data) ? todos.data : []));
              if (txs.data) {
                const filtered = Array.isArray(txs.data) ? txs.data.filter(tx => {
          ...
          // 장바구니 구매율 계산
          // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 장바구니 구매율 계산 로직 수정, 어디서: StatsPage.jsx 168-172번째 줄, 어떻게: totalQuantity와 purchasedQuantity로 purchaseRate 계산, 왜: 백엔드 API 응답 구조에 맞게 수정하기 위해
          if (!resCartCurrent?.data && resCartCurrent?.data?.totalQuantity > 0) {
            resCartCurrent.data.purchaseRate = (resCartCurrent.data.purchasedQuantity / resCartCurrent.data.totalQuantity) * 100;
          }
          
          if (!resTodoCurrent?.data && todoList.length > 0) {
            const totalTodos = todoList.length;
            const doneTodos = todoList.filter(todo => todo.isDone || todo.is_done).length;
            const completionRate = totalTodos > 0 ? (doneTodos / totalTodos) * 100 : 0;
            resTodoCurrent = { data: { totalTodos, completedTodos: doneTodos, completionRate } };
          }

          // 카테고리별 지출 통계 계산
          if (!resCategoryCurrent?.data?.categories || resCategoryCurrent.data.categories.length === 0) {
            ...
            if (totalExpense > 0) {
              ...
              resCategoryCurrent = { data: { categories: [], totalExpense: 0 } };
            }
          }
        }
        ...
        try {
          [resMealPrev, resCartPrev, resTodoPrev, resCategoryPrev] = await Promise.all([
            dataApi.get(`/api/stats/meal/range`, { params: { startDate: prevStart, endDate: prevEnd } }).catch(() => ({ data: null })),
            dataApi.get(`/api/stats/cart/range`, { params: { startDate: prevStart, endDate: prevEnd } }).catch(() => ({ data: null })),
            dataApi.get(`/api/todo/stats`, { params: { startDate: prevStart, endDate: prevEnd } }).catch(() => ({ data: null })),
            dataApi.get(`/api/tx/category-stats`, { params: { startDate: prevStart, endDate: prevEnd } }).catch(() => ({ data: { categories: [], totalExpense: 0 } }))
          ]);
        } catch (e) {
        ...
        if (!resMealPrev?.data || !resCartPrev?.data || !resTodoPrev?.data) {
          ...
          for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
            ...
            try {
              ...
              if (meals.data && meals.data.totalCalories !== undefined) {
                if (!resMealPrev?.data) {
                  resMealPrev = { data: { totalCalories: 0, targetCalories: 0 } };
                }
                resMealPrev.data.totalCalories += (Number(meals.data.totalCalories) || 0);
                resMealPrev.data.targetCalories += (Number(meals.data.targetCalories) || 2000);
              }
              
              if (cart.data && cart.data.totalQuantity !== undefined) {
                if (!resCartPrev?.data) {
                  resCartPrev = { data: { purchaseRate: 0, totalQuantity: 0, purchasedQuantity: 0 } };
                }
                resCartPrev.data.totalQuantity += (Number(cart.data.totalQuantity) || 0);
                resCartPrev.data.purchasedQuantity += (Number(cart.data.purchasedQuantity) || 0);
              }
          ...
          if (!resCartPrev?.data && resCartPrev?.data?.totalQuantity > 0) {
            resCartPrev.data.purchaseRate = (resCartPrev.data.purchasedQuantity / resCartPrev.data.totalQuantity) * 100;
          }
          ...
          if (!resTodoPrev?.data && todoList.length > 0) {
            ...
            const doneTodos = todoList.filter(todo => todo.isDone || todo.is_done).length;
            const completionRate = totalTodos > 0 ? (doneTodos / totalTodos) * 100 : 0;
            resTodoPrev = { data: { totalTodos, completedTodos: doneTodos, completionRate } };
          }

          if (!resCategoryPrev?.data?.categories || resCategoryPrev.data.categories.length === 0) {
            const categoryExpenses = {};
            const totalExpense = txList
              .filter(tx => tx.type === 'EXPENSE')
              .reduce((acc, tx) => {
        ...
        const calculateDietMealRate = async (startDate, endDate) => {
          ...
          const dateStatusMap = { normal: [], over: [], under: [], noRecord: [] }; // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 기록없음 날짜도 저장하도록 dateStatusMap에 noRecord 추가, 어디서: StatsPage.jsx 493번째 줄, 어떻게: dateStatusMap에 noRecord 배열 추가, 왜: 기록없음 항목에도 날짜를 표시하기 위해
            ...
            try {
              ...
              if (mealRes.data && Array.isArray(mealRes.data) && mealRes.data.length > 0) {
                ...
                const mealTypes = mealRes.data
                  .filter(meal => meal.mealType === '아침' || meal.mealType === '점심' || meal.mealType === '저녁')
                ...
                if (statsRes.data && statsRes.data.totalCalories !== undefined) {
          ...
          const mealRate = idealMeals > 0 ? Math.floor((totalMeals / idealMeals) * 100) : 0;
        ...
        let currentDietResult = { mealRate: 0, normalDays: 0, overDays: 0, underDays: 0 };
        let prevDietResult = { mealRate: 0, normalDays: 0, overDays: 0, underDays: 0 };
        
        // 현재 기간 식단 달성률 계산
        currentDietResult = await calculateDietMealRate(currentRange.startDate, currentRange.endDate);
        
        // 이전 기간 식단 달성률 계산
        prevDietResult = await calculateDietMealRate(prevRange.startDate, prevRange.endDate);
        ...
        if (resCategoryCurrent.data && resCategoryCurrent.data.categories) {
          currentConsumptionList = resCategoryCurrent.data.categories.map(item => ({
        ...
        if (resTodoCurrent.data) {
          currentScheduleValue = Math.floor(resTodoCurrent.data.completionRate || 0);
        }
        ...
        setCurrentDietDetail({ normalDays: currentDietResult.normalDays, overDays: currentDietResult.overDays, underDays: currentDietResult.underDays, totalPeriodDays: currentDietResult.totalPeriodDays || 0, dateStatusMap: currentDietResult.dateStatusMap || { normal: [], over: [], under: [], noRecord: [] } }); // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: dateStatusMap에 noRecord 추가, 어디서: StatsPage.jsx 617번째 줄, 어떻게: dateStatusMap 초기값에 noRecord 배열 추가, 왜: 기록없음 날짜를 표시하기 위해
        setPrevDietDetail({ normalDays: prevDietResult.normalDays, overDays: prevDietResult.overDays, underDays: prevDietResult.underDays, totalPeriodDays: prevDietResult.totalPeriodDays || 0, dateStatusMap: prevDietResult.dateStatusMap || { normal: [], over: [], under: [], noRecord: [] } }); // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: dateStatusMap에 noRecord 추가, 어디서: StatsPage.jsx 618번째 줄, 어떻게: dateStatusMap 초기값에 noRecord 배열 추가, 왜: 기록없음 날짜를 표시하기 위해
        ...
        setPrevDietScore(0);
        setCurrentDietDetail({ normalDays: 0, overDays: 0, underDays: 0, totalPeriodDays: 0, dateStatusMap: { normal: [], over: [], under: [], noRecord: [] } }); // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: dateStatusMap에 noRecord 추가, 어디서: StatsPage.jsx 631번째 줄, 어떻게: dateStatusMap 초기값에 noRecord 배열 추가, 왜: 기록없음 날짜를 표시하기 위해
        setPrevDietDetail({ normalDays: 0, overDays: 0, underDays: 0, totalPeriodDays: 0, dateStatusMap: { normal: [], over: [], under: [], noRecord: [] } }); // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: dateStatusMap에 noRecord 추가, 어디서: StatsPage.jsx 632번째 줄, 어떻게: dateStatusMap 초기값에 noRecord 배열 추가, 왜: 기록없음 날짜를 표시하기 위해
  ...
  const createDietPieData = (normalDays, overDays, underDays, dateStatusMap = { normal: [], over: [], under: [] }) => {
  ...
  const createConsumptionPieData = (consumptionData) => {
    ...
    return {
      ...
      datasets: [{
        ...
        backgroundColor: consumptionData.map((_, index) => colors[index % colors.length]),
        borderWidth: 0,
      }],
    };
  };
  ...
  const chart01Data = createDietPieData(currentDietDetail.normalDays, currentDietDetail.overDays, currentDietDetail.underDays, currentDietDetail.dateStatusMap);
  const chart02Data = createConsumptionPieData(currentConsumptionData);
  const chart03Data = createSinglePieData(currentCartScore, '장바구니 완료율', ['#ff9800', '#fff3e0']);
  const chart04Data = createSinglePieData(currentScheduleScore, '일정 달성률', ['#9c27b0', '#f3e5f5']);
  ...
  const getPeriodText = () => {
    ...
    if (periodType === 'week') {
      // [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 주 단위 날짜 표기를 줄바꿈 형식으로 변경, 어디서: StatsPage.jsx 812-818번째 줄, 어떻게: 시작 날짜 전체(yyyy-mm-dd)를 첫 줄에, ~ 기호를 두 번째 줄에, 종료 날짜 전체(yyyy-mm-dd)를 세 번째 줄에 표시, 왜: 사용자 요청에 따라 날짜 표기를 줄바꿈 형식으로 변경하기 위해
      return (
  ...
  return (
    <div className="stats-container">
      <header className="stats-header">
        ...
        <p className="stats-subtitle">기간별 활동을 분석했어요</p>
      </header>
      
      <div className="stats-main-content">
        {/* 왼쪽: 기간 선택 영역 (토글 + 날짜 선택기) */}
        {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 주/월 단위 토글과 날짜 선택기를 왼쪽에 배치하는 카테고리 형식으로 변경, 어디서: StatsPage.jsx 847-875번째 줄, 어떻게: stats-period-and-date-container를 왼쪽 사이드바 형식으로 배치, 왜: 사용자 요청에 따라 카테고리 형식으로 변경하기 위해 */}
        <div className="stats-sidebar">
          {/* 기간 단위 선택 토글 */}
          <div className="stats-period-toggle-container">
            <button 
              className={`stats-period-toggle-btn ${periodType === 'week' ? 'active' : ''}`}
              onClick={() => handlePeriodChange('week')}
            >
              주 단위
            </button>
            <button 
              className={`stats-period-toggle-btn ${periodType === 'month' ? 'active' : ''}`}
      ...
      <div className="stats-date-picker-container">
            <button className="stats-date-nav-btn" onClick={() => handlePeriodMove(-1)}>
          ◀
        </button>
        ...
        <div className="stats-pie-charts-container">
          {/* CHART 01: 식단관리 */}
          {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: CHART 01 라벨 삭제, 어디서: StatsPage.jsx 387-393번째 줄, 어떻게: stats-pie-chart-label div 제거, 왜: 사용자 요청에 따라 CHART 0n 문구 삭제하기 위해 */}
          <div className="stats-pie-chart-item">
            ...
            <div className="stats-pie-chart-detail">
              {currentDietDetail.normalDays > 0 && (
                <div className="stats-detail-item">
                  ...
                  {currentDietDetail.dateStatusMap?.normal && currentDietDetail.dateStatusMap.normal.length > 0 && (
                    <div className="stats-detail-dates">
                      ({currentDietDetail.dateStatusMap.normal.map(date => {
              ...
              {currentDietDetail.overDays > 0 && (
                <div className="stats-detail-item">
                  ...
                  {currentDietDetail.dateStatusMap?.over && currentDietDetail.dateStatusMap.over.length > 0 && (
                    <div className="stats-detail-dates">
                      ({currentDietDetail.dateStatusMap.over.map(date => {
                        const parts = date.split('-');
                        return parts.length === 3 ? `${parts[2]}일` : '';
                      }).filter(Boolean).join(',')})
                    </div>
              ...
              {currentDietDetail.underDays > 0 && (
                <div className="stats-detail-item">
                  ...
                  {currentDietDetail.dateStatusMap?.under && currentDietDetail.dateStatusMap.under.length > 0 && (
                    <div className="stats-detail-dates">
                      ({currentDietDetail.dateStatusMap.under.map(date => {
                        const parts = date.split('-');
                        return parts.length === 3 ? `${parts[2]}일` : '';
                      }).filter(Boolean).join(',')})
                    </div>
          ...
          {/* CHART 02: 소비율 (카테고리별) */}
          {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: CHART 02 라벨 삭제, 어디서: StatsPage.jsx 400-407번째 줄, 어떻게: stats-pie-chart-label div 제거, 왜: 사용자 요청에 따라 CHART 0n 문구 삭제하기 위해 */}
          <div className="stats-pie-chart-item">
            <div className="stats-pie-chart-wrapper">
              <Pie data={chart02Data} options={pieOptions} />
            </div>
            <div className="stats-pie-chart-title">소비율</div>
            {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 소비율 차트를 카테고리별 지출 비율로 변경, 어디서: StatsPage.jsx 393-397번째 줄, 어떻게: 카테고리별 지출 데이터를 파이 차트로 표시, 왜: 요구사항에 맞게 카테고리별 지출 비율을 시각화하기 위해 */}
            {currentConsumptionData.length > 0 && (
              <div className="stats-pie-chart-detail">
                {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 소비율 소수점 첫째 자리까지 표기, 어디서: StatsPage.jsx 570-575번째 줄, 어떻게: toFixed(1) 사용하여 소수점 첫째 자리까지 표시, 왜: 사용자 요청에 따라 소수점 표기를 첫째 자리까지로 제한하기 위해 */}
                {currentConsumptionData.slice(0, 3).map((item, index) => (
                  <div key={index} className="stats-detail-item">
                    {item.category}: {Number(item.percentage).toFixed(1)}%
                  </div>
                ))}
            </div>
            )}
          ...
          {/* CHART 03: 장바구니 완료율 */}
          {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: CHART 03 라벨 삭제, 어디서: StatsPage.jsx 419-426번째 줄, 어떻게: stats-pie-chart-label div 제거, 왜: 사용자 요청에 따라 CHART 0n 문구 삭제하기 위해 */}
          <div className="stats-pie-chart-item">
            <div className="stats-pie-chart-wrapper">
              <Pie data={chart03Data} options={pieOptions} />
            </div>
            <div className="stats-pie-chart-title">장바구니 완료율</div>
            {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 증감률 앞에 기간 문구 추가, 어디서: StatsPage.jsx 681-684번째 줄, 어떻게: periodType에 따라 '지난주 대비' 또는 '지난달 대비' 문구 추가, 왜: 사용자가 어떤 기간과 비교하는지 명확히 알 수 있도록 하기 위해 */}
            <div className="stats-change-rate">
              {periodType === 'week' ? '지난주 대비 ' : '지난달 대비 '}
              {calculateChangeRate(currentCartScore, prevCartScore) > 0 ? '+' : ''}
              {calculateChangeRate(currentCartScore, prevCartScore)}%
            </div>
          </div>

          {/* CHART 04: 일정 달성률 */}
          {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: CHART 04 라벨 삭제, 어디서: StatsPage.jsx 433-440번째 줄, 어떻게: stats-pie-chart-label div 제거, 왜: 사용자 요청에 따라 CHART 0n 문구 삭제하기 위해 */}
          <div className="stats-pie-chart-item">
            <div className="stats-pie-chart-wrapper">
              <Pie data={chart04Data} options={pieOptions} />
            </div>
            <div className="stats-pie-chart-title">일정 달성률</div>
            {/* [수정 2026-01-XX] 누가: 프론트엔드 개발자, 무엇을: 증감률 앞에 기간 문구 추가, 어디서: StatsPage.jsx 695-698번째 줄, 어떻게: periodType에 따라 '지난주 대비' 또는 '지난달 대비' 문구 추가, 왜: 사용자가 어떤 기간과 비교하는지 명확히 알 수 있도록 하기 위해 */}
            <div className="stats-change-rate">
              {periodType === 'week' ? '지난주 대비 ' : '지난달 대비 '}
              {calculateChangeRate(currentScheduleScore, prevScheduleScore) > 0 ? '+' : ''}
              {calculateChangeRate(currentScheduleScore, prevScheduleScore)}%
            </div>
          </div>
        </div>
      </div>
